# coding=utf-8
"""
@description: 车辆检索图形化界面主执行程序，选择图片传入，分析图片id，并从数据库中查找与图片id相同的车辆进行展示
@time：2019年11月30日19:36:21
@author：西安交通大学软件学院场景检索和检测组
@error: 因为服务器上的配置是2080ti，基本这种任务都能跑，但是在本地跑的时候要考虑显卡性能，所以在设备部分要修改
"""
import argparse
...
from ui_widget import Ui_TabWidget
from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtWidgets import QFileDialog, QTabWidget
from PyQt5.QtGui import QPixmap, QImage
import sys
import cv2
from db_opt import get_paths_by_id
...
# Pre_model用于标识是否使用预训练的模型，loacl_data_path用于指定本地文件路径
Pre_model = True
local_data_path = 'E:\\python_code\\xjtu\data\\VehicleID_V1.0'
...
# 单张图片数据读取, 用于获取图片的时候使用，要按照torch的形式读取图片数据
# 这个类中有初始化的方法，读取单张图片的方法以及获取数据长度
...
class SingleImgGUI():
    def __init__(self):
        resume = local_data_path + '\\epoch_14.pth'
        data_root = local_data_path
        self.net = RepNet(out_ids=10086, out_attribs=257).to(device)
        # print('=> Mix difference network:\n', net)
        # 从断点启动. 如果指定了训练好的模型的话,就将模型参数输入进去
        # 从现在来看，相当于是调用的时候，重新执行了这里的东西
        if resume is not None:
            print("加载训练好的模型epoch14...")
            if os.path.isfile(resume):
                # 加载模型
                self.net.load_state_dict(torch.load(resume))
                print('=> net resume from {}'.format(resume))
                print("模型加载完毕!")
            else:
                print('=> [Error]: invalid resume path @ %s' % resume)
        ...
        colorID2name_path = data_root + '/attribute/colorID2name.pkl'
        trainID2Vid_path = data_root + '/attribute/trainID2Vid.pkl'
        if not (os.path.isfile(modelID2name_path) and os.path.isfile(colorID2name_path) and os.path.isfile(
                (trainID2Vid_path))):
            print('=> [Err]: invalid file.')
        with open(modelID2name_path, 'rb') as fh_1, \
                open(colorID2name_path, 'rb') as fh_2, \
                open(trainID2Vid_path, 'rb') as fh_3:
            self.modelID2name = pickle.load(fh_1)
            self.colorID2name = pickle.load(fh_2)
            self.trainID2Vid = pickle.load(fh_3)

    # Pre_model = False
    # print(Pre_model)
    # 第二个版本，不显示具体的图像，只是进行单纯的操作
    # 将网络的加载防止在全局
    # 加载网络, 分别是输出的id数据和属性,然后放在gpu上, 并将网络结构进行输出,这边直接都给他搞成全局的方便进行显示
    # if Pre_model:
    def get_vid_by_image(self, img_path):
        # 颜色字典
        ...
        # assert(1 == 2)
        test_loader = torch.utils.data.DataLoader(dataset=single_img,
                                                  batch_size=1,
                                                  shuffle=False,
                                                  num_workers=1)
        ...
        # index_data = enumerate(test_loader)
        # data = index_data[0].data
        # label = index_data[0].label
        for i, (data, label) in enumerate(test_loader):
            # 放入GPU.
            data, label = data.to(device), label.to(device).long()
            # 前向运算: 预测车型、车身颜色, 其实感觉这部分的label可以加入到下一步的操作中
            output_attrib = self.net.forward(X=data, branch=1, label=None)
            pred_mc = get_predict_mc(output_attrib).cpu()[0]
            pred_m_id, pred_c_id = pred_mc[0].item(), pred_mc[1].item()
            pred_m_name = self.modelID2name[pred_m_id]
            pred_c_name = self.colorID2name[pred_c_id]

            # 前向运算: 预测Vehicle ID, 我在想, 我要是任意模仿一波单边的数据会出现什么情况呢
            # 在预测ID的时候将车辆ID传进去, 这个操作可也太迷了
            output_id = self.net.forward(X=data, branch=3, label=label)
            _, pred_tid = torch.max(output_id, 1)
            pred_tid = pred_tid.cpu()[0].item()
            pred_vid = self.trainID2Vid[pred_tid]
...
def show_imgs(imgs):
    paths = []
    # root_path = '/home/asus/design_biye/target_detection/rid_data/dataset/VehicleID_V1.0/image/'
    root_path = local_data_path + '\\image\\'
    for path in imgs:
        path = root_path + path
        paths.append(path)
...
class mywindow(QTabWidget, Ui_TabWidget):
    ...
    def open_file(self):
        ...
        if img_name:
            # 读取图片并预测车辆的id信息
            vid = self.mymodel.get_vid_by_image(img_name)
            # vid = 345
            print("预测中...")
            vid = str(vid)
            print(vid)
            # result = getresult(img_name)
            self.label_img.setPixmap(jpg)
            # 从数据库中读取数据，完成查询操作
            imgs = get_paths_by_id(int(vid))
            # imgs = ['0003799.jpg', '0003811.jpg', '0003816.jpg', '0003800.jpg']
            # 显示图片
            imgs_len = len(imgs)
            if imgs_len == 0:
                self.label_result.setText('车辆ID预测结果为%s, 但是在数据库中为查询到匹配车辆' % vid)
            elif imgs_len > 0:
                self.label_result.setText('车辆ID预测结果为%s, 查询到%d条匹配记录' % (vid, imgs_len))
                show_imgs(imgs)

    # 显示图片,其实传入的是图像名称，将图像转化为QPixmap的形式
...
# 函数调用，通过函数形式完成功能
def func_call(img_name):
    mymodel = SingleImgGUI()
    vid = mymodel.get_vid_by_image(img_name)
    print("预测中...")
    vid = str(vid)
    print(vid)
    imgs = get_paths_by_id(int(vid))
...
if __name__ == '__main__':
    # 因为目前是全局的模式，会导致模型反复启动，所以要改成类的形式，仅加载模型一次
    main_gui_start()
    # func_call('E:\\python_code\\xjtu\\vehicle_search_xjtu\\imgs\\0000003.jpg')
